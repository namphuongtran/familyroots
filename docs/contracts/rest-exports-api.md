...
| `relationship_type != "biological"` (e.g. `adopted`) | `2 PEDI <relationship_type>` under that child's `FAMC` line for that family unit — no separate `ADOP` tag (the birth-family `FAMC` carries no PEDI) |
