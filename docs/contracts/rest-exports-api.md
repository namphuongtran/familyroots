...
| Parent-child edges | Children attach via `1 FAMC @Fn@` on the child. A child's parents are grouped **by `relationship_type`** and a FAM is **never** formed across types (a biological father is never emitted as HUSB/WIFE of an adoptive father — M6). Within one type: the child attaches to an existing **marriage** FAM if two of that type's parents are married; else exactly **two** parents of that type form one synthetic couple FAM; else each parent gets a single-parent FAM. A child therefore has one `FAMC` per family unit (e.g. a biological/birth family plus an adoptive family), all deterministic (children, types, and parents sorted) |
| `relationship_type` `adopted`/`foster` | `2 PEDI adopted`/`2 PEDI foster` under that child's `FAMC` line for that family unit — no separate `ADOP` tag (the biological/birth-family `FAMC` carries no PEDI, GEDCOM's implied default) |
| `relationship_type == "step"` | `step` is **not** a valid GEDCOM 5.5.1 `PEDI` value (`{adopted, birth, foster, sealing}`), so it is emitted as a `2 NOTE FamilyRoots: step-parent relationship` under the `FAMC` instead of an invalid `2 PEDI step` |
