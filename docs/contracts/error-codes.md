...
| `relationship.duplicate_marriage` | 409 | Active marriage between the two persons already exists in this clan (checked on create, and on a PATCH that flips `status` to `married`, excluding the marriage being updated) | — | Refresh |
| `relationship.duplicate_spouse_order` | 409 | Two of `person1`'s active (`status = married`) marriages would share the same `spouse_order` — checked on create/update when `spouse_order` is set or `status` flips to `married`, and backstopped by the partial unique index `uq_marriages_spouse_order` for races (a raw `23505` at that index is caught by the generic integrity-error handler and returned as `conflict` (409), not this code — clients should treat both as retryable conflicts) | — | Ask for a different vợ cả/vợ hai/... order, or refresh to see the current ordering |
