...
This plan was authored from documentation, not from a running system. The backend
observability plan that preceded it shipped three defects of exactly that origin, so
the following claims were checked against reality before execution. **Treat anything
not on this list as unverified** — verify library APIs against the installed package
before trusting the code in a task.

Verified true:

- Every file and export this plan references exists on `main`: `createClient` (async)
  in `src/lib/supabase/server.ts`, `createClientOrNull` in `src/lib/supabase/client.ts`,
  `src/app/[locale]/layout.tsx`, the `@/*` → `./src/*` tsconfig alias, and a `<form>`
  on the login page (Task 11's smoke test asserts it). `docs/sad/05b-web-components.md`
  and `docs/sad/11-risks-and-technical-debt.md` are present since SAD PR #121 merged.
- `openapi-typescript` 7.13.0 declares `export = openapiTS` and exposes `astToString`
  as a named export; under ESM the default import in Task 2's script resolves to the
  function. `import openapiTS, { astToString }` works.
- Task 1's Vitest config — `test.projects` with `extends: true` — runs unchanged on
  Vitest **4.1.10** (smoke-tested: config + one test, passed).
- Task 10's dependency-cruiser config shape works on **18.1.0**: the `domain-is-pure`
  rule fires on a `react` import inside `src/domain/`, and the binary exits 1 on a
  violation and 0 on a clean tree, so CI gating works.
- `@sentry/nextjs` **10.69.0** exports `withSentryConfig` and `captureRequestError` from
  the server entry, and `captureRouterTransitionStart` from the client entry
  (`build/esm/index.client.js`) — which is what `instrumentation-client.ts` resolves to
  under the `browser` export condition. Task 8's usage is correct.
- `AbortSignal.any` (Task 7) exists on Node 20+, which is what CI pins.

Version pins were refreshed to current majors on 2026-08-02: Vitest `^3`→`^4`,
vite-tsconfig-paths `^5`→`^6`, jsdom `^25`→`^30`, jest-dom `^6`→`^7`, plugin-react
`^4`→`^6`, dependency-cruiser `^16`→`^18`. `openapi-typescript@^7`,
`@testing-library/react@^16`, `@testing-library/user-event@^14`, `msw@^2` and
`@playwright/test@^1.49` were already current.

**Not yet verified** — check before relying on them: MSW 2.15 handler API against
Vitest 4 + jsdom 30; `@testing-library/react` 16 with React 19 under jsdom 30;
Next.js 16 + Sentry 10 build integration; whether `next/web-vitals`'
`useReportWebVitals` signature still matches Task 8.

**Note on the `depcruise` script name:** the binary is `depcruise` but the package is
`dependency-cruiser`. `pnpm depcruise` resolves the local bin and is safe; never run
`npx depcruise` in a tree where the dev dependency is absent — a package literally
named `depcruise` exists on npm as a dependency-confusion placeholder.
...
```bash
cd web && pnpm add -D vitest@^4 vite-tsconfig-paths@^6 jsdom@^30
```
...
```bash
cd web && pnpm add -D @testing-library/react@^16 @testing-library/user-event@^14 @testing-library/jest-dom@^7 @vitejs/plugin-react@^6 msw@^2
```
