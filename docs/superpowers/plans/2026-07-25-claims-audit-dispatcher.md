...
- [ ] **Step 4: FOLDED-IN FIX — approve/reject post-commit MissingGreenlet (see spec Addendum).** `approve_claim` and `reject_claim` end `await self._uow.commit()` then `return IdentityClaimResponse.model_validate(claim)`. `IdentityClaimResponse` declares `updated_at` (server `onupdate=func.now()`), which is expired after the UPDATE → `model_validate` async-lazy-loads it → MissingGreenlet → 500 despite a successful commit. Fix: between the commit and the `model_validate`, add `await self._uow.session.refresh(claim, attribute_names=["updated_at", "created_at"])` so the expired timestamps are re-fetched inside the async context (targeted attribute list keeps relationships loaded). Apply to BOTH `approve_claim` and `reject_claim` only (submit INSERTs → fine; cancel returns None). Verify `self._uow.session` is the right accessor for the AsyncSession.
- [ ] **Step 5: Test-tolerance cleanup** — now that approve/reject return a clean 200: (a) in `backend/tests/integration/test_claims_audit.py`, remove the `try/except PydanticValidationError` around the approve call — assert a clean `resp.status_code == 200`; (b) in `backend/tests/integration/test_claim_approval.py`, remove the mislabeled "test-harness artifact" tolerance around the direct `approve_claim` call — assert it now returns normally (a real `IdentityClaimResponse`). Both were masking the same real bug this task fixes.
- [ ] **Step 6: Run** — Task-1 file green (ip/UA now populated + approve asserts clean 200); `test_claim_approval.py` green (clean return); existing claims + audit suites green; FULL suite (report count). mypy: check per-module overrides. If an existing test asserted `add_audit` was called (mock), update to assert the dispatched audit row instead — justify.
- [ ] **Step 7: Commit** — `git commit -m "fix(claims): route audit through fail-closed dispatcher (+ip/UA); cancel audits unconditionally; fix approve/reject post-commit MissingGreenlet 500; drop direct add_audit (M12)"`.
