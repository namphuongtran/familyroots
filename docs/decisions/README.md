...
| [056](056-next-public-api-url-splits-into-a-browser-and-a-server-variable.md) | `NEXT_PUBLIC_API_URL` Splits Into a Browser Variable and a Server Variable | Accepted, shipped (2026-08-22, seed S-076), opened by S-071. `NEXT_PUBLIC_API_URL` keeps the browser-facing job it always had; a new, unprefixed `API_URL` is read live by `auth-context.ts` for the server-to-server call, falling back to `NEXT_PUBLIC_API_URL` **so Vercel — the app's real deploy target — needs no second variable**. Rejects a same-origin Next.js proxy as **already forbidden** by `.claude/rules/nextjs.md:64-65`. A real-browser control shows the old compose default failed outright with `net::ERR_NAME_NOT_RESOLVED` where the corrected one reaches a real socket, `net::ERR_CONNECTION_REFUSED` — two different failure codes, so the name-resolution difference is measured rather than argued |
| [057](057-the-invitation-link-is-the-primary-join-path.md) | The Invitation Link Is the Primary Join Path, and the Clan Code Is the Secondary One | Accepted (2026-08-26, seed S-080), opened by the maintainer from using the register screen. The admin shares a **browser** URL, `https://<origin>/<locale>/invitations/<token>`, whose origin is its own variable and explicitly **not** `NEXT_PUBLIC_API_URL` (ADR-056 owns that name). The typed join field becomes the **slug**, resolved through the `get_clan_by_slug` lookup that already exists at every layer, and `placeholder="UUID"` is deleted. Records two findings the seed did not have: accept requires an authenticated caller (`backend/app/api/v1/invitations.py:95-99`), and `clan_action` is required on registration (`backend/app/schemas/auth.py:18,25`), so **the invitation flow is unreachable end to end for a new user today** — opened as S-085. Rejects public clan discovery against ADR-044 and `CLAUDE.md:40` |
