...
/**
 * A 2026-08-22 deletion removed six legacy person components and, with them, every caller of
 * 44-by-the-coordinator's-count message keys across `member`, `member_form`, `members`, and the
 * wholly-dead `relationship_form` namespace. That deletion was fenced out of `web/messages/*.json` for
 ...
 *
 * This file lives outside `web/src` on purpose: a palette sweep was running over `web/src` the same batch,
 ...
 *
 * **Widened to the whole file on 2026-08-22.** This test was first scoped to three
 ...
 * `relationship`, `document`, `event`, `common`, and `auth`. That drift predated the first version and a
 * global assertion would have failed on it immediately, for a reason that version was not sized to fix.
 * The widening is that fix: it added the 61 missing keys, with real Chinese and French translations, not
 ...
 * `.claude/rules/testing.md`'s "a test pins an outcome, not a setting". Do not narrow this back down
