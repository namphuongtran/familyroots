...
import type { TreeApiResponse, TreeAncestorsResponse, RelationshipPath, ApiResponse } from '@/lib/types'
...
export const treeApi = {
  ...
  getAncestors: async (
    personId: string,
    maxGenerations = 10,
  ): Promise<TreeAncestorsResponse> => {
    const { data } = await api.get<ApiResponse<TreeAncestorsResponse>>(`/tree/ancestors/${personId}`, {
      params: { max_generations: maxGenerations },
    })
    return data.data
  },
