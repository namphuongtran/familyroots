  ...
  return (
    <div className="space-y-1">
      <p className="text-sm font-medium text-gray-700">
        {t('path_from')} <span className="text-primary-600">{fromName}</span> {t('path_to')}{' '}
        <span className="text-primary-600">{toName}</span>
      </p>
      <ol className="list-none space-y-0.5">
        {steps.map((step, i) => (
          <li key={i} className="flex items-center gap-2 text-sm text-gray-600">
            <span className="w-4 h-4 flex items-center justify-center rounded-full bg-primary-100 text-primary-700 text-[10px] font-bold shrink-0">
              {i + 1}
            </span>
            <span>
              {step.full_name}
