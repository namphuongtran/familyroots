...
def _make_person(mid=None, birth_date=None):
...
async def test_self_loop_prevented_by_schema():
    ...
    from app.schemas.parent_child import ParentChildCreateRequest
...
async def test_too_many_bio_parents(validator, clan_id):
    ...
    parent = _make_person(birth_date=date(1970, 1, 1))
    child = _make_person(birth_date=date(2000, 1, 1))
...
async def test_parent_too_young(validator, clan_id):
    ...
    parent = _make_person(birth_date=date(2000, 1, 1))
    child = _make_person(birth_date=date(2005, 1, 1))
...
async def test_cycle_detection(validator, clan_id):
    ...
    parent = _make_person(birth_date=date(1950, 1, 1))
    child = _make_person(birth_date=date(1980, 1, 1))
...
@pytest.mark.asyncio
async def test_duplicate_marriage(validator):
    """Cannot create a duplicate active marriage between the same two persons."""
    ...
    with pytest.raises(ConflictError, match="duplicate_marriage"):
        await validator.check_duplicate_marriage(person1_id, person2_id, db)
