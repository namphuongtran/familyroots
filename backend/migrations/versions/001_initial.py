...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "generation IS NULL OR generation > 0", name="members_generation_positive"
        ),
    )
    ...
    op.execute(
        "CREATE INDEX idx_members_is_deleted ON members "
        "(clan_id, is_deleted) WHERE is_deleted = false"
    )
    op.create_index("idx_members_birth_date", "members", ["clan_id", "birth_date"])
    op.execute(
        "CREATE INDEX idx_members_is_founder ON members (clan_id) WHERE is_clan_founder = true"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "relation_type",
            sa.Enum("parent", "child", "spouse", name="relation_type", create_type=True),
            nullable=False,
        ),
        sa.Column(
            "relation_subtype",
        ...
        sa.CheckConstraint(
            "(relation_type IN ('parent', 'child') "
            "AND relation_subtype IN ('biological','adoptive','step','foster')) "
            "OR (relation_type = 'spouse' "
            "AND relation_subtype IN ('married','divorced','widowed','partner'))",
    ...
    op.create_index(
        "idx_relationships_member", "relationships", ["clan_id", "member_id", "relation_type"]
    )
    op.create_index(
        "idx_relationships_related", "relationships", ["clan_id", "related_id", "relation_type"]
    )
    ...
    op.execute(
        "CREATE INDEX idx_documents_member ON documents (member_id) WHERE member_id IS NOT NULL"
    )
