...
**Three guards watch `scripts/` and `backend/scripts/`, and they read three different things. Do not widen one to cover another's subject.** `tests/unit/test_scripts_sql_is_sanctioned.py` (S-069) reads `.sql` files. `tests/unit/test_inline_sql_in_scripts_is_sanctioned.py` (S-077) reads the SQL a `psql` invocation executes — `-c`, heredocs, here-strings. `tests/unit/test_script_cli_privilege_is_sanctioned.py` (S-079) reads **argv**, because a client CLI needs no SQL text at all: `createuser --superuser familyroots_app` escalates exactly as `ALTER ROLE … SUPERUSER` does, and measured 2026-08-26 both SQL guards stay green with that line planted in a script. All three share their sanction shape — an allow-list keyed by path, a class set per entry, and a privilege check that **no** entry can override — and they share code where sharing prevents drift: S-077 imports S-069's classifier, and S-079 imports S-077's file sweep so the two cannot disagree about which script exists.
