...
- **`clan_settings` is the case where nothing read the table at all, and one selectin still had to be checked.** Migration `035` (Phase 10, S-010) gives it the 027 template. Measured 2026-08-22: nothing in `app/` reads or writes it except `Clan.settings` (`app/models/clan.py:35`), whose result no caller consumes, and **nothing anywhere constructs a `ClanSettings`**, so the table is empty and the policy is inert. But `Clan.settings` is `lazy="selectin"`, so `get_clan_by_slug` and `get_clan_by_id` emit a SELECT against it on the request session with **no clan GUC** during register and onboard. It returns nothing and both flows still answer `201`. `Clan` declares five `lazy="selectin"` relationships (`clan.py:32-36`) and three of those targets already carried policies, so that pattern was already load-bearing. **On an empty table, a denial test proves nothing on its own**: every one in `tests/integration/test_rls_phase10_clan_settings.py` ends with a privileged read proving the rows were there.
