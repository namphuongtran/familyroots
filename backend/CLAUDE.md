...
2. Clan isolation is enforced in the **application/repository layer**: every clan-scoped read takes `clan_id` as an explicit filter (the PRIMARY guarantee). RLS layer-2 (SP-3, ADR-008) is **active for `documents`, `events`, `branches`, `parent_child`, `marriages`, `persons`, `change_requests`, `clan_memberships`, and `clan_invitations`** (Phases 1-7, migrations 002/026-032): request sessions drop to the non-bypass `familyroots_app` role + set the `app.clan_id` GUC per transaction (`app/core/rls.py`); system sessions bypass. Gated by `RLS_ENABLED`. Other tables roll out table-by-table.

**A tenth table has RLS enabled and no clan isolation, and the two are not in tension.** `identity_claims` (Phase 8, migration `033`, ADR-042, seed S-012) carries one policy, `identity_claims_system_session_only FOR ALL USING (false) WITH CHECK (false)`. It compares nothing to the GUC. It is a **tripwire** for a claims query mis-wired to `get_db`; it does not catch a missing `created_by_clan_id` filter on the correct session, and the application layer is that table's only clan isolation. So **"is RLS on and is there a policy" is not a coverage question any more.** The guard in `tests/integration/test_rls_activation.py` is split into `_CLAN_ISOLATED_TABLES` and `_REQUEST_ROLE_DENIED_TABLES`, and `test_each_half_of_the_rls_set_matches_what_its_policies_do` asserts each half separately: clan-isolated tables need a policy whose `USING` reads `app.clan_id`, denied tables need every policy reading `USING (false)`/`WITH CHECK (false)`. Do not move a name between the sets to make a test pass — measured 2026-08-22, a policy flipped to `USING (true) WITH CHECK (true)` still passes the older coverage assertion and is caught only by the split one.
...
- **`user_clan_roles` cannot take a clan policy as it stands.** `get_current_clan_id` reads it to decide which clan is active (`app/core/security.py:246-253`, GUC set only at `:290`), and so do `get_login_profile` (`auth_repository.py:118-135`) and `GET /me/clans` (`me_query_port.py:19-42`). With a policy applied, login returns `200` with `clan_id: null` and `/me/clans` returns `[]`. Pinned by `tests/integration/test_rls_login_two_clans.py`.
