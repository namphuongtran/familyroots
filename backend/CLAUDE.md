...
**After a commit, an ORM instance you UPDATEd is behind the row, and reading it raises rather than returning a stale value.** `expire_on_commit=False` (`app/core/database.py:52,69`) means the commit itself expires nothing, so this looks safe and mostly is. The exception is a column whose new value only the database knows: `TimestampMixin.updated_at` carries `onupdate=func.now()` (`app/models/base.py:34-39`), a SQL expression. SQLAlchemy 2.0's default `eager_defaults="auto"` adds `RETURNING` on **INSERT only, never on UPDATE**, so after an UPDATE flush that one attribute is left unloaded; touching it post-commit starts a lazy load with no greenlet and raises `MissingGreenlet`. Under Pydantic it surfaces as `ValidationError: Error extracting attribute` on `updated_at` — the model name in the message is the response schema, not the cause. **The failure is intermittent in the way that hides it**: a no-op PATCH emits no UPDATE, expires nothing, and answers 200, so a smoke test that PATCHes the same value twice passes forever. **And the row is written** — the client gets a 500 plus a successful write, which is the pair that makes retrying unsafe. Measured 2026-08-26 on `PATCH /clans/me`: `assert (500, 'Ăn quả nhớ kẻ trồng cây') == (200, 'Ăn quả nhớ kẻ trồng cây')`.

The remedy in this repository is `await uow.session.refresh(model, attribute_names=["updated_at", "created_at"])` after the commit — `app/application/clan/handlers.py:69` and, since before it, `app/application/person/claim_handlers.py:186,227`. **Name the attributes**: a bare `refresh()` expires the relationships too, and `Clan` declares four `lazy="selectin"` ones (`app/models/clan.py:32-35`). The repository-level counterpart, for write paths that emit a Core UPDATE and must resync the identity map, is `session.refresh(existing)` at `person_repository.py:212`, `event_repository.py:236`, `change_request_repository.py:112`, and `relationship_repository.py:183,223`.

**Only one route was ever exposed to this, and the reason tells you where to look next.** Every other write path serializes the **domain entity**, which is plain Python and sets its own `updated_at` in `update()` (e.g. `app/domain/person/entity.py:191`) — no IO, nothing to expire. `app/domain/clan/entity.py` has no timestamp fields at all and `clan_mapper.to_domain` does not copy them, so the clan response had to come off the ORM row. **So the question to ask of a new write path is not "does it commit" but "does it read an ORM instance the same transaction UPDATEd".**
...
**Fourteen tables have RLS enabled and the fully covered count is eleven, because three of them carry policies that are not clan isolation — in three different ways.** `identity_claims` (Phase 8, migration `033`, ADR-042) carries one policy, `identity_claims_system_session_only FOR ALL USING (false) WITH CHECK (false)`. It compares nothing to the GUC. It is a **tripwire** for a claims query mis-wired to `get_db`; it does not catch a missing `created_by_clan_id` filter on the correct session, and the application layer is that table's only clan isolation. `audit_logs` (Phase 9, migration `034`, ADR-043) is clan-keyed on **reads only**: `audit_logs_sel USING (clan_id = <GUC>)`, `audit_logs_ins WITH CHECK (true)`, and **no UPDATE or DELETE policy**, which denies both commands to the request role and makes the trail append-only at the database. The permissive INSERT is not an oversight — `POST /auth/register` is unauthenticated and `POST /auth/onboard` resolves no clan, yet both write an audit row on the request session, so a clan-keyed `WITH CHECK` compares `<real clan> = NULL` and rejects registration.
...
**A new revision must be added to `../docs/ops/migrations.md` § "Current chain", and that file's `Head = ` line updated, in the same commit.** `tests/unit/test_migrations_doc_matches_alembic.py` derives both sides at run time — the chain from `alembic.script.ScriptDirectory`, the claimed chain from the document — and fails on any difference: a revision on disk the document does not name, a revision the document names that is not on disk, a different order, or a stale head. It hardcodes no revision and no head, so it needs no edit when one lands; it simply starts requiring the document to name it. Added 2026-08-22, after `docs/ops/migrations.md:119` on `main` at `62a863d` read "Head = `035_rls_clan_settings`" while `036_rls_user_clan_roles.py` had already shipped a batch earlier, and nothing caught it. The document is the source an agent reads first and the migration files are the source of truth, so when the two disagree, fix the document.

**The migration scripts are part of the shipped distribution, not just the source tree.** `pyproject.toml` lists `migrations` in `[tool.hatch.build.targets.wheel] packages`, `migrations/__init__.py` is what makes that possible, and `app/core/readiness.py` finds them with `importlib.resources.files("migrations")`. Never locate them by counting parents from a module's `__file__`. That is what a 2026-08-26 change removed: `Path(__file__).resolve().parents[2]` is correct in the source tree and wrong in `Dockerfile`'s `uv sync --no-editable` layout, where the module lands in `site-packages`. The whole suite stayed green while the production image could not boot at all. `tests/unit/test_wheel_carries_migration_scripts.py` builds the wheel, unpacks it, and reads the head with only the wheel on the import path — a source-tree assertion cannot see this class of defect, and `tests/unit/test_readiness.py::test_expected_head_reads_migration_scripts` is the one that proved it by staying green.
...
**Three guards watch `scripts/` and `backend/scripts/`, and they read three different things. Do not widen one to cover another's subject.** `tests/unit/test_scripts_sql_is_sanctioned.py` reads `.sql` files. `tests/unit/test_inline_sql_in_scripts_is_sanctioned.py` reads the SQL a `psql` invocation executes — `-c`, heredocs, here-strings. `tests/unit/test_script_cli_privilege_is_sanctioned.py` reads **argv**, because a client CLI needs no SQL text at all: `createuser --superuser familyroots_app` escalates exactly as `ALTER ROLE … SUPERUSER` does, and measured 2026-08-26 both SQL guards stay green with that line planted in a script. All three share their sanction shape — an allow-list keyed by path, a class set per entry, and a privilege check that **no** entry can override — and they share code where sharing prevents drift: the inline-SQL guard imports the `.sql` guard's classifier, and the argv guard imports the inline-SQL guard's file sweep, so the two cannot disagree about which script exists.

Three things to know before touching the argv one. **Flag tables are keyed by binary**, because `pg_restore -c` is `--clean` while `psql -c` is `--command`, and `pg_restore -S NAME` names an existing superuser rather than granting anything — a shared table would misread `gunzip -c "$DUMP" | pg_restore …` on the shipped tree. **It asks two questions with two reaches**: checks 1 and 2 read only the command word, so `echo "==> pg_restore completed"` is not counted, while check 3 reads every token in the segment so `sudo -u postgres createuser -s app` cannot hide behind a wrapper. **It resolves one literal level of indirection and it is a drift guard, not a security boundary**: `CU=createuser; $CU -s app` is caught, `CU=cre; ${CU}ateuser -s app` is not, and that miss is planted and asserted in `test_the_indirection_boundary_is_where_this_module_says_it_is` rather than only described. The module's own "What this misses" list is the authority; the largest live hole is that `scripts/supabase_local.sh:100` runs `supa stop --no-backup`, which its own comment at `:98` says DELETES the Supabase database, through a shell function wrapping a non-Postgres CLI that no guard reads.
