...
2. Clan isolation is enforced in the **application/repository layer**: every clan-scoped read takes `clan_id` as an explicit filter (the PRIMARY guarantee). RLS layer-2 (SP-3, ADR-008) is **active for `documents`, `events`, `branches`, `parent_child`, `marriages`, `persons`, `change_requests`, `clan_memberships`, `clan_invitations`, `notification_log`, `clan_settings`, and `user_clan_roles`** (Phases 1-11, migrations 002/026-036): request sessions drop to the non-bypass `familyroots_app` role + set the `app.clan_id` GUC per transaction (`app/core/rls.py`); system sessions bypass. Gated by `RLS_ENABLED`. Other tables roll out table-by-table.
