...
async def get_supabase_jwks() -> dict:
    """Fetch Supabase public JWKS for JWT signature verification.

    .. note::
        TODO: implement in Prompt 2 — cache JWKS response to avoid
...
async def get_current_clan_id(
    ...
    approved_clan_ids = [row[0] for row in result.fetchall()]
    ...
    if x_current_clan_id is not None:
        try:
            ...
            raise HTTPException(status_code=400, detail="Invalid X-Current-Clan-Id format")
